...
export default function CartView() {
  ...
  if (items.length === 0) {
    return (
      <div className="mx-auto max-w-6xl px-4 py-10">
        <h1 className="mb-8 border-b border-black/10 pb-6 text-4xl font-semibold dark:border-white/10">Your Bag</h1>
        <div className="py-24 text-center">
          ...
          <Link
            href="/tshirts"
            className="inline-block border border-black px-8 py-3 text-xs font-semibold uppercase tracking-widest hover:bg-black hover:text-white transition-colors dark:border-white dark:hover:bg-white dark:hover:text-black"
          >
            Continue Shopping
          </Link>
        </div>
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      <h1 className="mb-8 border-b border-black/10 pb-6 text-4xl font-semibold dark:border-white/10">
        Your Bag{" "}
      ...
      <div className="grid grid-cols-1 gap-12 lg:grid-cols-[1fr_380px]">
            ...
            <div key={`${item.product.id}-${item.selectedSize}-${item.selectedColor}`} className="flex gap-5 py-6">
              ...
              <div className="flex flex-1 flex-col justify-between">
                ...
                <div className="flex items-center justify-between">
                  {/* Quantity */}
                  <div className="flex h-8 w-24 items-center border border-black/15 dark:border-white/20">
                    <button
                      onClick={() => updateQuantity(item.product.id, item.selectedSize, item.selectedColor, item.quantity - 1)}
                      className="flex h-full w-8 items-center justify-center text-sm hover:bg-zinc-50 dark:hover:bg-zinc-800"
                    >–</button>
                  ...
                  <button
                    ...
                    className="text-[11px] text-zinc-400 underline underline-offset-2 hover:text-black transition-colors dark:hover:text-white"
                  >
                    Remove
                  </button>
                </div>
        ...
        <div className="border border-black/10 p-6 h-fit dark:border-white/10">
          ...
          {!promoCode && (
            <form onSubmit={handlePromo} className="mb-5 flex gap-2">
              <input
                ...
                className="flex-1 border border-black/15 px-3 py-2 text-xs outline-none focus:border-black dark:border-white/20 dark:focus:border-white"
              />
              <button
                type="submit"
                className="border border-black px-4 py-2 text-[10px] font-semibold uppercase tracking-widest hover:bg-black hover:text-white transition-colors dark:border-white dark:hover:bg-white dark:hover:text-black"
              >
                Apply
              </button>
            </form>
          ...
          {promoSuccess && (
            <p className="mb-3 text-xs text-green-700 dark:text-emerald-400">
          ...
          <Link
            ...
            className="mt-6 flex h-12 w-full items-center justify-center bg-black text-xs font-semibold uppercase tracking-widest text-white hover:bg-zinc-800 transition-colors dark:bg-white dark:text-black dark:hover:bg-zinc-200"
          >
            Proceed to Checkout
          </Link>
